...
lemma lemma1 (n : ℕ) (hn : n = 0) (x : ℝ) (hx : 1 ≤ x)
  ...
  have ha2_le : a^2 ≤ x^2 := by rw [sq_le_sq₀]; exact ha_le; norm_cast; exact Nat.zero_le a; exact hx_nonneg
  ...
  have floor_x2_eq : ⌊x^2⌋ = a^2 := by
    obtain ⟨m2, hm2_pos, hm2_sq⟩ := h 2 (by simp [hn])
  ...
  have floor_x4_eq : ⌊x^4⌋ = a^4 := by
    obtain ⟨m4, hm4_pos, hm4_sq⟩ := h 4 (by simp [hn])
    rw [← hm4_sq, (show 4 = 2 * 2 by norm_num), pow_mul, sq_eq_sq₀ (Nat.cast_nonneg m4) (by norm_num), ← floor_x2_eq]
